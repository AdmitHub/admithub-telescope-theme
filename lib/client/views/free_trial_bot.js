...
Template.freeTrialBot.helpers({
  ...
  showBeginBotTracker: function(step) {
    var show = step === 2 && !Session.get("beginBotTrackerShown");
    if (show) {
      Session.set("beginBotTrackerShown", true);
      console.log("Show begin bot tracker");
    }
    return show;
  },
  showParentContactInfoTracker: function(step, doc) {
    var show = (doc.parent1Email || doc.parent1Phone) &&
      !Session.get("parentContactInfoTrackerShown");
    if (show) {
      Session.set("parentContactInfoTrackerShown", true);
      console.log("Show parent contact info tracker");
    };
    return show;
  },
