...
if (Meteor.isClient) {
  Meteor.startup(function() {
    ...
    Router.route("/survey/parent/:step?", {
      name: "parentSurvey",
      template: "survey_unsupported",
      onBeforeAction: filters.isLoggedIn,
      waitOn: waitOnLogin,
    });

    Router.route("/survey/educator/:step?", {
      name: "educatorSurvey",
      template: "survey_unsupported",
      onBeforeAction: filters.isLoggedIn,
      waitOn: waitOnLogin,
    ...
    Router.route("/survey/student/:step?", {
      ...
      data: function() {
        ...
        if (!this.params.step || isNaN(parseInt(this.params.step))) {
          // TODO: Advance to "next" step instead of first.
          Router.go("/survey/student/1", null, {replaceState: true});
        }
      ...
      action: function() {
        if (this.ready()) {
          if (!Meteor.user()) {
            ...
            /*
            // TODO: Deal with complexity of user segmentation.
            var u = Meteor.user();
            var tag = UserTags.findOne(u.profile.tags[0]);
            if (tag) {
              if (tag.slug === "student") {
                return Router.go("/survey/student/", null, {replaceState: true});
              } else if (tag.slug === "parent") {
                return Router.go("/survey/parent/", null, {replaceState: true});
              } else if (tag.slug === "educator") {
                return Router.go("/survey/educator/", null, {replaceState: true});
              }
            }
            Router.go("/");
            flashMessage("Sorry, an error occurred.", "error");
            */
          }
        }
      }
